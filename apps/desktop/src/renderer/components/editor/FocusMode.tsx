    ...
    <div className="fixed inset-0 bg-surface z-50 flex flex-col">
      <div className="h-10 bg-surface-light flex items-center justify-between px-4 shrink-0">
        <div className="flex items-center gap-4 text-xs text-white/40">
          ...
          <span>📝 {sessionWords} 字</span>
          {wph > 0 && <span>⚡ {wph} 字/时</span>}
          <span>🎯 {todayWritten}/{dailyGoal}</span>
          {targetWords > 0 && <span>📊 {wordCount}/{targetWords} ({Math.min(100, Math.round((wordCount / targetWords) * 100))}%)</span>}
        </div>
